...
import { Filter, ChevronLeft, ChevronRight, SlidersHorizontal, X, Search, Scale, CheckCircle2, ChevronDown, ChevronUp } from 'lucide-react';
...
export const CarsPage: React.FC = () => {
  ...
  // Mobile/Tablet (< 1024px): Closed by default
      ...
      <div className="relative container mx-auto px-4 h-full flex flex-col justify-center items-center text-center z-10 pt-20 md:pt-0">
        <div className="max-w-3xl space-y-4 animate-fade-in-up px-4">
          ...
          <p className="text-lg md:text-xl text-gray-200 font-medium max-w-2xl">
            {t('cars.hero.subtitle', { country: selectedCountry?.name || 'Qatar' })}
          </p>
        </div>
      </div>
    </div>
    <div className="flex flex-col lg:flex-row gap-8 relative items-start">
      ...
      `}>
            <div className="p-6 pb-20 lg:pb-6">
                <div className="flex items-center justify-between mb-6 pb-4 border-b border-gray-100 dark:border-gray-700 sticky top-0 bg-white dark:bg-gray-800 z-10 pt-2 lg:static lg:bg-transparent lg:pt-0">
                ...
                <div className="space-y-6">
                    {/* Search Input */}
                    <div>
                    <label className={filterLabelClass}>{t('hero.search')}</label>
      ...
      <div className="flex-grow min-w-0">
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6 gap-4 bg-white dark:bg-gray-800 p-4 rounded-xl shadow-sm border border-gray-100 dark:border-gray-700">
           <div className="flex items-center gap-4">
              ...
              <div>
                <h1 className="text-2xl font-bold dark:text-white leading-tight">{t('nav.cars')}</h1>
                <p className="text-xs text-gray-500 mt-1">
                    {t('cars.showing_results', { count: cars.length, total: totalCount })}
                </p>
              </div>
           </div>
           ...
           <div className="flex flex-wrap gap-3 w-full sm:w-auto">
              ...
              <button 
                ...
                className={`flex-1 sm:flex-none flex items-center justify-center gap-2 px-4 py-2 rounded-lg font-bold text-sm transition shadow-sm border ${
                  isCompareMode 
                  ...
                  {getPageNumbers().map((p, idx) => (
                    typeof p === 'number' ? (
                      <button
                        ...
                        className={`w-10 h-10 rounded-lg text-sm font-bold transition shadow-sm border ${
                          page === p 
                            ? 'bg-primary-600 text-white border-primary-600' 
