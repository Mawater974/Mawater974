...
import { CheckCircle, XCircle, Star, Eye, Search, Archive, SlidersHorizontal, Edit, RefreshCw, ExternalLink } from 'lucide-react';
...
export const AdminCarsPage: React.FC = () => {
  ...
  const handleView = (car: Car, e?: React.MouseEvent) => {
      ...
      // Determine country code from car data or fallback to 'qa'
      // Need to cast to any because country relation might return an object
      const countryCode = (car.countries as any)?.code?.toLowerCase() || 'qa';
      window.open(`/#/${countryCode}/cars/${car.id}`, '_blank');
  };
                        ...
                        >
                           ...
                           <td className="px-6 py-4">
                              <div className="flex justify-end gap-2" onClick={(e) => e.stopPropagation()}>
                                 ...
                                 {car.status !== 'archived' && (
                                     <button onClick={(e) => handleDelete(car.id, e)} title="Archive" className="p-2 text-gray-400 hover:text-red-500 hover:bg-gray-100 rounded-lg">
