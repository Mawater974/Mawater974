...
import { getCars, updateCarStatus, updateCarFeatured, deleteCar, exportToCSV, SortOption } from '../../services/dataService';
import { Car } from '../../types';
import { CheckCircle, XCircle, Star, Eye, Search, Archive, SlidersHorizontal } from 'lucide-react';
...
export const AdminCarsPage: React.FC = () => {
  ...
  const fetchCars = async () => {
    ...
    const { data } = await getCars(
        { status: filter === 'all' ? undefined : filter, searchQuery: search }, 
        1, 
        50,
        sortBy
    );
  ...
  const handleFeaturedToggle = async (id: number, current: boolean) => {
            ...
            <table className="w-full text-left text-sm">
               ...
               <tbody className="divide-y divide-gray-100 dark:divide-gray-700">
                  {loading ? (
                     ...
                     cars.map(car => (
                        <tr key={car.id} className={`hover:bg-gray-50 dark:hover:bg-gray-700/50 transition ${car.status === 'archived' ? 'opacity-60' : ''}`}>
                           ...
                           <td className="px-6 py-4">
                              <button onClick={() => handleFeaturedToggle(car.id, car.is_featured)} className={`p-1 rounded-full ${car.is_featured ? 'text-yellow-500 bg-yellow-50' : 'text-gray-300 hover:text-yellow-500'}`}>
                           ...
                           <td className="px-6 py-4">
                              <div className="flex justify-end gap-2">
                                 {car.status === 'pending' && (
                                    <>
                                       <button onClick={() => handleStatusChange(car.id, 'approved')} title="Approve" className="p-2 text-green-600 hover:bg-green-50 rounded-lg">
                                          <CheckCircle className="w-5 h-5" />
                                       </button>
                                       <button onClick={() => handleStatusChange(car.id, 'rejected')} title="Reject" className="p-2 text-red-600 hover:bg-red-50 rounded-lg">
                                          <XCircle className="w-5 h-5" />
                                       </button>
                                    </>
                                 )}
                                 <Link to={`/cars/${car.id}`} target="_blank" title="View" className="p-2 text-blue-600 hover:bg-blue-50 rounded-lg">
                                    <Eye className="w-5 h-5" />
                                 </Link>
                                 
                                 {car.status !== 'archived' && (
                                     <button onClick={() => handleDelete(car.id)} title="Archive" className="p-2 text-gray-400 hover:text-red-500 hover:bg-gray-100 rounded-lg">
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 backdrop-blur-sm p-4">
            <div className="bg-white dark:bg-gray-800 rounded-2xl w-full max-w-4xl max-h-[90vh] overflow-y-auto shadow-2xl animate-fade-in-up">
                <div className="flex justify-between items-center p-6 border-b border-gray-100 dark:border-gray-700 sticky top-0 bg-white dark:bg-gray-800 z-10">
                    <h2 className="text-xl font-bold dark:text-white">Add New Listing (Admin Mode)</h2>
