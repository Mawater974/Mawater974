...
import { getSpareParts, deleteSparePart, exportToCSV, getOptimizedImageUrl, updateSparePart } from '../../services/dataService';
import { SparePart } from '../../types';
import { CheckCircle, XCircle, Search, Archive, Edit, Settings, RefreshCw, Eye, ExternalLink } from 'lucide-react';
...
export const AdminPartsPage: React.FC = () => {
  ...
  const handleView = (part: SparePart, e?: React.MouseEvent) => {
      ...
      // Determine country code from part data or fallback to 'qa'
      const countryCode = (part.countries as any)?.code?.toLowerCase() || 'qa';
      window.open(`/#/${countryCode}/parts/${part.id}`, '_blank');
  };
                           ...
                           <td className="px-6 py-4">
                              <div className="flex justify-end gap-2" onClick={(e) => e.stopPropagation()}>
                                 ...
                                 {part.status !== 'archived' && (
                                     <button onClick={(e) => handleDelete(part.id, e)} title="Archive" className="p-2 text-gray-400 hover:text-red-500 hover:bg-gray-100 rounded-lg">
