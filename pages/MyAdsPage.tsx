...
export const MyAdsPage: React.FC = () => {
  ...
  const handleDeleteCar = async (carId: number) => {
      if (confirm("Are you sure you want to delete (archive) this ad? It will be hidden from public view.")) {
  ...
  const handleDeletePart = async (partId: string) => {
      if (confirm("Are you sure you want to delete (archive) this part? It will be hidden from public view.")) {
  ...
  return (
    <div>
      ...
      <div className="flex gap-4 mb-8 border-b border-gray-200 dark:border-gray-700">
          ...
          >
              <CarIcon className="w-5 h-5" /> Cars ({myCars.length})
          </button>
          <button 
          ...
          >
              <Settings className="w-5 h-5" /> Spare Parts ({myParts.length})
          </button>
      </div>
                ...
                myCars.length > 0 ? (
                    ...
                    {myCars.map(car => (
                        <div key={car.id} className="relative group">
                            <CarCard car={car} language={language} t={t} />
                            
                            {/* Action Buttons Overlay */}
                            <div className="absolute top-2 left-2 flex gap-2 z-20 opacity-0 group-hover:opacity-100 transition">
                                <button 
                                    onClick={(e) => { e.preventDefault(); setEditingCar(car); }}
                            ...
                            <div className="absolute top-2 right-2 bg-white/90 text-gray-800 text-xs font-bold px-2 py-1 rounded z-20">
                    ...
                    <div className="text-center py-20 bg-white dark:bg-gray-800 rounded-xl">
                        ...
                        <p className="text-gray-500 text-lg">You haven't posted any car ads yet.</p>
                    </div>
                )
            )}
                ...
                myParts.length > 0 ? (
                    ...
                    {myParts.map(part => (
                        <div key={part.id} className="relative group">
                            <SparePartCard part={part} language={language} t={t} />
                            <div className="absolute top-2 left-2 flex gap-2 z-20 opacity-0 group-hover:opacity-100 transition">
                                <button 
                                    onClick={(e) => { e.preventDefault(); handleEditPart(part.id); }}
                                    className="bg-blue-500 hover:bg-blue-600 text-white p-2 rounded-full shadow-lg"
                                    title="Edit"
                                >
                                    <Edit className="w-4 h-4" />
                                ...
                                <button 
                                    onClick={(e) => { e.preventDefault(); handleDeletePart(part.id); }}
                                    className="bg-red-500 hover:bg-red-600 text-white p-2 rounded-full shadow-lg"
                                    title="Delete / Archive"
                                >
                                    <Trash2 className="w-4 h-4" />
                    ...
                    <div className="text-center py-20 bg-white dark:bg-gray-800 rounded-xl">
                        ...
                        <p className="text-gray-500 text-lg">You haven't posted any spare part ads yet.</p>
                    </div>
                )
            )}
        </>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 overflow-y-auto">
              <div className="bg-white dark:bg-gray-900 w-full max-w-4xl rounded-2xl shadow-2xl overflow-hidden my-auto relative max-h-[90vh] flex flex-col">
                  <div className="p-6 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center sticky top-0 bg-white dark:bg-gray-900 z-10">
                      <h2 className="text-xl font-bold dark:text-white">Edit Listing</h2>
      ...
      {editingPart && (
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4 overflow-y-auto">
              <div className="bg-white dark:bg-gray-900 w-full max-w-4xl rounded-2xl shadow-2xl overflow-hidden my-auto relative max-h-[90vh] flex flex-col">
                  <div className="p-6 border-b border-gray-100 dark:border-gray-800 flex justify-between items-center sticky top-0 bg-white dark:bg-gray-900 z-10">
                      <h2 className="text-xl font-bold dark:text-white">Edit Part</h2>
