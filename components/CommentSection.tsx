  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-sm border border-gray-100 dark:border-gray-700 mt-8">
      ...
      <div className="space-y-6 mb-8">
        {comments.length === 0 ? (
          <p className="text-gray-500 text-center py-4">No comments yet. Be the first to ask!</p>
        ) : (
          comments.map(comment => (
            <div key={comment.id} className="flex gap-4">
              <div className="flex-shrink-0 w-10 h-10 rounded-full bg-gray-100 dark:bg-gray-700 flex items-center justify-center text-gray-500">
                {comment.profiles?.avatar_url ? (
                  <img src={comment.profiles.avatar_url} alt="" className="w-full h-full rounded-full object-cover" />
                ) : (
                  <span className="font-bold">{comment.profiles?.full_name?.[0] || <User className="w-5 h-5" />}</span>
                )}
              </div>
              <div className="flex-grow">
                 <div className="flex justify-between items-start">
                    <div>
                        <span className="font-bold text-gray-900 dark:text-white mr-2">{comment.profiles?.full_name || 'User'}</span>
      ...
      {user ? (
        <form onSubmit={handleSubmit} className="flex gap-4">
           <div className="flex-shrink-0 hidden md:block w-10 h-10 rounded-full bg-primary-100 text-primary-600 flex items-center justify-center font-bold">
        ...
        <div className="bg-gray-50 dark:bg-gray-900 p-4 rounded-xl text-center">
            <p className="text-sm text-gray-600 dark:text-gray-400 mb-2">Please login to post a comment.</p>
            <Link to={`/${countryCode}/login`} className="text-primary-600 font-bold hover:underline">Login / Register</Link>
        </div>
      )}
    </div>
  );
